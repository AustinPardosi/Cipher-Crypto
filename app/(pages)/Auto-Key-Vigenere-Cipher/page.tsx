...
const AutoKeyVigenereChiper = () => {
  ...
  const handleEncrypt = async (inputContent: String) => {
    try {
      const respone = await fetch("/api/auto-key-vigenere/encrypt", {
        method: "POST",
  ...
  const handleDecrypt = async (inputContent: String) => {
    try {
      const respone = await fetch("/api/auto-key-vigenere/decrypt", {
        method: "POST",
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify({ text: inputContent, key: key }),
      });
      ...
      <div className="max-w-8xl mx-auto">
        <div className="flex gap-4">
          ...
          <h1 className="text-4xl text-[#F244B5] font-bold mb-4">
            Vigenère Cipher: Auto Key Variant
          </h1>
        </div>

        {/* Description of Algorithm */}
        <p className="text-lg mb-6">
          Vigenere Cipher Auto Key is a method of encrypting alphabetic text. It uses a simple form of polyalphabetic substitution. The key is generated from key and plaintext iff teh key is not long enough, and is used to encrypt the plaintext.
        </p>
...
export default AutoKeyVigenereChiper;
