...
const PlayfairCipher = () => {
  ...
  const handleEncrypt = async (inputContent: String) => {
    try {
      const respone = await fetch("/api/playfair/encrypt", {
        method: "POST",
  ...
  const handleDecrypt = async (inputContent: String) => {
    try {
      const respone = await fetch("/api/playfair/decrypt", {
        method: "POST",
        headers: { "Content-Type": "application/json; charset=utf-8" },
        body: JSON.stringify({ text: inputContent, key: key }),
      });
  ...
  return (
    <main className="min-h-screen bg-[#121212] text-[#E0E0E0] p-8 font-sans">
      <div className="max-w-8xl mx-auto">
        ...
        <p className="text-lg mb-6">
          This chipper only accepts lowercase alphabets [a-z] and will ignore any non-alphabetic characters.
        </p>
